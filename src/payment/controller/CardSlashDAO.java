...
public class CardSlashDAO implements InterCardSlashDAO {
	...
	public CardSlashVO cardSelectList(String fk_userno, String cartno) throws SQLException {
		...
		try {
			...
			String sql = " SELECT fk_userno, cartno, fk_productcode, c.pprice, poqty, pprice * poqty AS total_price, fk_optioncode, PRODUCTIMG1, KRPRODUCTNAME, PRODUCTDESCSHORT FROM tbl_product p RIGHT JOIN tbl_cart c ON p.productcode = c.fk_productcode "
	...
	public int prePayLog(Map<String, String> paraMap) throws SQLException {
		...
		// insert into TBL_PAYMENT_LOG_DETAIL ( FK_ORDER_NO, FK_USERNO, OPTION_CODE,
		// QTY, CLIENTIP )
	...
	public List<CardSlashVO> selectAddressList(String puserno) throws SQLException {
		...
		try {
			...
			String sql = " select ship_seq, receiverName, siteName, postcode, address, detailAddress, extraAddress, mobile, deliveryRequest, status "
					+ " from tbl_shipping " + " where fk_userno = ? " + " and status = 1 order by status desc ";
...
//    System.out.println("증가할 포인트 : " + point);
	...
	@Override
	public int cartUpdate(String userno, String optioncode) throws SQLException {
