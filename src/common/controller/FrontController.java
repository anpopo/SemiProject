...
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		 ...
		 * 않는다. 딱 1번만 init(ServletConfig config) 메소드에는 FrontController 서블릿이 동작해야할 환경설정을
		 * 잡아주는데 사용된다.
		 */

		// *** 확인용 *** //
		// System.out.println("~~~ 확인용 => 서블릿 FrontController 의 init(ServletConfig
		// config) 메소드가 실행됨.");
		...
		// Properties는 Collection 중 HashMap 계열중의 하나로써
		// "key","value"으로 이루어져 있는것이다.
		// 그런데 중요한 것은 Properties는 key 도 String 타입이고, value도 String 타입만 가능하다는 것이다.
		...
		try {
			...
			// ~~~ 확인용 props =>
			// C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties

			fis = new FileInputStream(props);
			// fis 는 C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의
			// 내용을 읽어오기 위한 용도로 쓰이는 객체이다.

			pr.load(fis);
			/*
			 * pr.load(fis); 은 fis 객체를 사용하여
			 * C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의 내용을
			 * 읽어다가 Properties 클래스의 객체인 pr에 로드시킨다. 그러면 pr은 읽어온 파일(Command.properties)의 내용에서
			 * = 을 기준으로 좌측은 key, 우측은 value로 인식한다.
			 */

			Enumeration<Object> en = pr.keys();
			/*
			 * pr.keys();은 C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties
			 * 파일의 내용을 읽어다가 = 을 기준으로 좌측의 모든 key들만 가져오는 것이다.
			 */
			while (en.hasMoreElements()) {
				...
				if (className != null) {
					...
					// <?>는 generic 인데 어떤 클래스 타입인지는 모르지만 하여튼 클래스 타입이 들어온다.
			...
			System.out.println(">>> C:/NCS/workspace(jsp)/Covengers/WebContent/WEB-INF/Command.properties 파일이 없습니다.");
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
		} catch (Exception e) {
	...
	private void requestProcess(HttpServletRequest request, HttpServletResponse response)
		...
		// 웹브라우저의 주소 입력창에서
		// http://localhost:9090/MyMVC/member/idDuplicateCheck.up?userid=leess 와 같이
		// 입력되었더라면
		 String url = request.getRequestURL().toString();
		 System.out.println("[0] 확인용 url => " + url);
		// ~~~ 확인용 url => http://localhost:9090/MyMVC/member/idDuplicateCheck.up

		String uri = request.getRequestURI();
		 System.out.println("[0] 확인용 uri => " + uri);
		 System.out.println();
		...
		// /MyMVC/member/idDuplicateCheck.up
		// /main.up
		// /index.up
